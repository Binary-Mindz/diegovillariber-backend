...
export class AdminLegalNoticeService {
  ...
  async create(dto: CreateLegalNoticeDto) {
    ...
    if (dto.targetType === LegalNoticeTarget.CAR) {
      if (!dto.carId)
        throw new BadRequestException('carId is required for CAR legal notice');
      if (dto.bikeId)
        throw new BadRequestException(
          'bikeId must be null for CAR legal notice',
        );
    }
    ...
    if (dto.targetType === LegalNoticeTarget.BIKE) {
      if (!dto.bikeId)
        throw new BadRequestException(
          'bikeId is required for BIKE legal notice',
        );
      if (dto.carId)
        throw new BadRequestException(
          'carId must be null for BIKE legal notice',
        );
    }
    ...
    if (dto.targetType === LegalNoticeTarget.BIKE) {
      ...
      if (bike.profileId !== dto.profileId)
        throw new ForbiddenException(
          'Bike does not belong to the provided profile',
        );
    }
    ...
    return this.prisma.legalNotice.create({
      ...
      include: {
        ...
        car: {
          select: { id: true, displayName: true, make: true, model: true },
        },
        bike: {
          select: { id: true, displayName: true, make: true, model: true },
        },
