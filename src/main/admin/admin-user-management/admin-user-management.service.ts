...
import {
  ...
  PostModerationQueryDto,
} from './dto/post-mpderation.query.dto';
...
export class AdminUserManagementService {
    ...
    const verifiedUsers = await this.prisma.user.count({
      where: {
        ...
        profile: {
          some: {
            activeType: {
              in: [Type.PRO_DRIVER, Type.PRO_BUSSINESS, Type.CONTENT_CREATOR],
            },
          },
        },
      },
    ...
    const userTypeDistribution = grouped
      .map((g) => ({
        ...
        percent:
          totalTyped === 0
            ? 0
            : Number(((g._count._all / totalTyped) * 100).toFixed(1)),
      }))
  ...
  async listPostsForModeration(
    ...
    const where: Prisma.PostWhereInput = {
      ...
      ...(q
        ? {
            OR: [{ caption: { contains: q, mode: QueryMode.insensitive } }],
          }
        : {}),
    };
    ...
    const profileIds = [
      ...new Set(posts.map((p) => p.profileId).filter(Boolean)),
    ] as string[];
  ...
  async deleteUser(userId: string) {
    ...
    if (user.role === Role.ADMIN) {
      throw new NotFoundException(
        'Admin user cannot be deleted from this endpoint',
      );
    }
