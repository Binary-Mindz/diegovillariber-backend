...
export class HeadToHeadService {
  ...
  async getBattle(id: string) {
    const battle = await this.prisma.headToHeadBattle.findUnique({
      ...
      include: {
        ...
        _count: {
          select: { participants: true, submissions: true, invitations: true },
        },
      },
    });
      ...
      if (dto.accessType === BattleAccessType.AUTO_INVITE) {
        if (!dto.autoInviteScope || !dto.autoInviteCount) {
          throw new BadRequestException(
            'autoInviteScope and autoInviteCount are required when accessType = AUTO_INVITE',
          );
        }
      }
  ...
  async updateBattle(
    ...
    return this.prisma.headToHeadBattle.update({
      ...
      data: {
        ...
        ...(dto.battleCategory !== undefined
          ? { battleCategory: dto.battleCategory }
          : {}),
        ...(dto.brandFilter !== undefined
          ? { brandFilter: dto.brandFilter }
          : {}),
        ...(nextDurationDays !== undefined
          ? { durationDays: nextDurationDays }
          : {}),

        ...(dto.winPrize !== undefined ? { winPrize: dto.winPrize } : {}),
        ...(dto.uploadImageOrVideo !== undefined
          ? { uploadImageOrVideo: dto.uploadImageOrVideo }
          : {}),

        ...(dto.cameraRequirement !== undefined
          ? { cameraRequirement: dto.cameraRequirement }
          : {}),
        ...(dto.requireTrueShotVerified !== undefined
          ? { requireTrueShotVerified: dto.requireTrueShotVerified }
          : {}),
        ...
        ...(dto.autoInviteScope !== undefined
          ? { autoInviteScope: dto.autoInviteScope }
          : {}),
        ...(dto.autoInviteCount !== undefined
          ? { autoInviteCount: dto.autoInviteCount }
          : {}),
  ...
  async joinBattle(battleId: string, userId: string) {
    ...
    if (battle.accessType === BattleAccessType.INVITATION_ONLY) {
      ...
      if (!inv || inv.status !== InvitationStatus.ACCEPTED) {
        throw new ForbiddenException(
          'Invitation required (and must be accepted) to join this battle',
        );
      }
    }
    ...
    if (battle.accessType === BattleAccessType.FOLLOWERS_ONLY) {
      ...
      throw new ForbiddenException(
        'FOLLOWERS_ONLY access requires follower check implementation',
      );
    }
  ...
  async respondInvitation(
    ...
    const inv = await this.prisma.battleInvitation.findUnique({
      where: { id: invitationId },
    });
    if (!inv) throw new NotFoundException('Invitation not found');
    if (inv.inviteeId !== userId)
      throw new ForbiddenException('Only invitee can respond');
  ...
  async submit(battleId: string, userId: string, dto: SubmitHeadToHeadDto) {
    ...
    if (!participant || participant.status !== ParticipantStatus.JOINED) {
      throw new ForbiddenException(
        'You must join the battle before submitting',
      );
    }
  ...
  async vote(submissionId: string, userId: string, dto: VoteHeadToHeadDto) {
    ...
    if (submission.battle.status !== BattleStatus.ACTIVE) {
      throw new BadRequestException(
        'Voting is allowed only when battle is Active',
      );
    }
  ...
  async createComment(
    battleId: string,
    userId: string,
    dto: CreateHeadToHeadCommentDto,
  ) {
    ...
    if (dto.submissionId) {
      const sub = await this.prisma.battleSubmission.findUnique({
        where: { id: dto.submissionId },
      });
      if (!sub || sub.battleId !== battleId)
        throw new BadRequestException('Invalid submissionId for this battle');
    }
  ...
  async completeBattle(battleId: string, userId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (submissions.length === 0)
        throw new BadRequestException('No submissions found to pick winner');
