...
export class DynamicTranslationInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      mergeMap(async (response) => {
        ...
        if (response.data) {
          response.data = await this.translateDeep(
            response.data,
            lang,
            context,
          );
  ...
  private async translateDeep(
    ...
    if (Array.isArray(data)) {
      return Promise.all(
        data.map((item) => this.translateDeep(item, lang, context)),
      );
    }
        ...
        if (prismaClient.translation) {
          ...
          for (const field of this.translatableFields) {
            if (
              ...
              const matchedTranslation = existingTranslations.find(
                (t: any) => t.field === field,
              );
              ...
              if (matchedTranslation) {
                ...
                await prismaClient.translation
                  .create({
                    data: {
  ...
  private guessEntityType(context: ExecutionContext): string {
    ...
    if (url.includes('post-rating') || url.includes('rating'))
      return 'PostRating';
    ...
    if (url.includes('rawshift-battle') || url.includes('rawshift'))
      return 'RawShiftBattle';
    if (url.includes('rawshift-entry')) return 'RawShiftEntry';
    if (url.includes('rawshift-comment')) return 'RawShiftComment';

    if (url.includes('splitscreen-match') || url.includes('match-request'))
      return 'SplitScreenMatchRequest';
    if (url.includes('splitscreen-battle') || url.includes('splitscreen'))
      return 'SplitScreenBattle';

    if (url.includes('challenge') || url.includes('program'))
      return 'Challenge';
    if (
      url.includes('head-to-head') ||
      url.includes('h2h') ||
      url.includes('battle')
    )
      return 'HeadToHeadBattle';

    // 3. Marketplace & Products
    if (url.includes('wishlist')) return 'WishList';
    if (url.includes('product') || url.includes('marketplace'))
      return 'ProductList';

    // 4. Profiles & Racing Modules
    if (url.includes('pro-driver') || url.includes('pro-profile'))
      return 'ProDriverProfile';
    if (url.includes('sim-racing') || url.includes('simracing'))
      return 'SimRacingProfile';
    ...
    if (
      url.includes('submit-lap-time') ||
      url.includes('labtime') ||
      url.includes('laptime')
    )
      return 'SubmitLabTime';
    if (url.includes('setup-description')) return 'SetupDescriptionPhoto';

    // 6. Spotting Module
    if (url.includes('spotting-match')) return 'SpottingMatch';
    if (url.includes('sportting-request') || url.includes('spotting-request'))
      return 'SpottingRequest';

    // 7. Garage, Cars & Tuning (Advanced Car Data)
    if (url.includes('virtual-garage')) return 'VirtualGarage';
    if (url.includes('virtual-event') || url.includes('sim-event'))
      return 'VirtualSimRacingEvent';
    if (url.includes('garage')) return 'Garage';
    if (url.includes('car')) return 'Car';
    if (url.includes('bike')) return 'Bike';
    if (url.includes('tuning') || url.includes('tuning-aero'))
      return 'TuningAero';
    ...
    if (url.includes('prestige-rule') || url.includes('prestige'))
      return 'PrestigeRule';
    if (url.includes('prize')) return 'Prize';
    if (url.includes('tutorial')) return 'Tutorial';
    if (url.includes('user-point') || url.includes('points'))
      return 'UserPoint';
    if (url.includes('notification')) return 'Notification';
